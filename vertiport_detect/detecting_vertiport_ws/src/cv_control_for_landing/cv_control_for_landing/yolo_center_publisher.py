...
class YOLOCenterPublisher(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(
            Image, "/camera/realsense_front/color/image_raw", self.image_callback, 10
        )
    ...
    def image_callback(self, msg):
        ...
        for box in results.boxes:
            ...
            if (
                0 <= cy < self.latest_depth_image.shape[0]
                and 0 <= cx < self.latest_depth_image.shape[1]
            ):
            ...
            # 출력
            self.get_logger().info(
                f"🎯 객체: {cls_name} | 중심: ({cx}, {cy}) | 깊이: {depth_in_m:.2f} m"
            )
